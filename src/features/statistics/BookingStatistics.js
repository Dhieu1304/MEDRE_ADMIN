...
function BookingStatistics() {
  ...
  const [series, categories, xLabel, yLabel] = useMemo(() => {
    const seriesData = [
      {
        name: t("chart.seriName.totalOnl"),
        data: data?.map((item) => parseInt(item.totalOnl, 10) || 0)
      },
      {
        name: t("chart.seriName.totalOff"),
        data: data?.map((item) => parseInt(item.totalOff, 10) || 0)
      }
    ];
                ...
                >
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell>
                        <Typography variant="subtitle1">{t("table.col.totalOff")}</Typography>
                      </TableCell>
                      <TableCell>
                        <Typography variant="subtitle1">{t("table.col.totalOnl")}</Typography>
                      </TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {data?.map((item) => {
                      return (
                        <TableRow key={item?.time}>
                          ...
                          <TableCell sx={{ textAlign: "right" }}>{item?.totalOnl || 0}</TableCell>
                          <TableCell sx={{ textAlign: "right" }}>{item?.totalOff || 0}</TableCell>
                        </TableRow>
                      );
                    })}
                  </TableBody>
